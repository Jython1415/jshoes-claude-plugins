...
**Purpose**: Ensures proper authorship attribution when Claude creates git commits, pull requests, issues, or comments on behalf of the user.
...
- Includes 10-minute cooldown to avoid repetitive suggestions
...
- Within 10-minute cooldown period since last suggestion
...
- 600-second (10-minute) cooldown period
...
- Cooldown may prevent guidance on subsequent operations within 10 minutes
...
### Test Coverage for gh-authorship-attribution

The `gh-authorship-attribution.py` hook has 41 tests covering:
- ✅ Git commit detection: Basic commits, heredoc commits, amend, various flags
- ✅ Attribution detection: Recognizes Co-authored-by, AI-assisted, session links, etc.
- ✅ GitHub API detection: POST/PATCH to pulls, issues, comments endpoints
- ✅ Attribution in API bodies: Detects existing attribution in request bodies
- ✅ Cooldown mechanism: 10-minute cooldown prevents duplicate suggestions
- ✅ Cooldown applies across operation types: Git and API share cooldown state
- ✅ Non-triggering commands: git status/add/push/log/diff don't trigger
- ✅ Non-GitHub operations: curl to non-GitHub URLs don't trigger
- ✅ GET requests: Read-only GitHub API calls don't trigger
- ✅ Tool filtering: Non-Bash tools (Read, Write, Edit) don't trigger
- ✅ Edge cases: Malformed JSON, missing fields, very long commands handled
- ✅ JSON validity and event name correctness
- ✅ Output validation: Guidance includes examples and transparency messaging
- ✅ Case-insensitive detection: Git commit detection works regardless of case
- ✅ Chained commands: Detects git commit in command chains
